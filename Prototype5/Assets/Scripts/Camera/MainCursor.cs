	...
	void  OnGUI () {
		...
		if(Input.GetMouseButton(0)){
			...
			if(CameraMoving.showUpCursor==false && CameraMoving.showDownCursor==false && CameraMoving.showRightCursor==false && CameraMoving.showLeftCursor==false){
				...
				GUI.DrawTexture (new Rect(Event.current.mousePosition.x-cursorSizeX/2, Event.current.mousePosition.y-cursorSizeY/2, cursorSizeX, cursorSizeY), this.cursorTexture);
